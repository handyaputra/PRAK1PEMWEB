...
if($tmp === false)
	err(404,'404! File or Directory Not Found');
...
if($_GET['do'] == 'list') {
	...
	foreach($disallowed_extensions as $ext)
		if(preg_match(sprintf('/\.%s$/',preg_quote($ext)), $_FILES['file_data']['name']))
			err(403,"403! Files of this type are not allowed.");
...
// source: http://php.net/manual/en/function.realpath.php#84012
	...
	function list() {
		...
		$.get('?do=list&file='+ hashval,function(data) {
			...
			if(data.success) {
				...
				!data.results.length && $tbody.append('<tr><td class="empty" colspan=5>Direktori ini kosong</td></tr>')
...
<div id="top">
   <?php if($allow_create_folder): ?>
	<form action="?" method="post" id="mkdir" />
		<label for=dirname>Buat Folder Baru</label><input id=dirname type=text name=name value="" />
